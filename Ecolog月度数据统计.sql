...
sum(case when currentnodetype=3 then 1 else 0 end) as '归档',
sum(case when currentnodetype=3 then 0 else 1 end) as '未归档',
sum(case when hours>=15*24 then 1 else 0 end) as '超过15天',
sum(case when thcs=0 or thcs is null then 0 else 1 end) as '退回流程数量',
sum(thcs) as '退回次数'
from
(
select (select workflowtype from workflow_base where id=workflowid) as workflowtype,
...
from workflow_requestbase w
where w.createdate>='2016-01-01' and w.createdate<'2016-11-30'
) t
