--drop table material_history_item_log
...
select * into material_history_item_log from openquery([RF],'select * from v_material_history_item_log where createDate<=''2019-03-18 16:00''')
...
,A.charg as '批次',A.sonum_ex as '特殊库存编码',A.bestq as '库存类型',B.verme as '库存数量',B.quantity as '盘点数量',B.盘点时间,B.username as '掌机号'
from [dbo].[stomtr] A
left join (

select left(createDate,19) as '盘点时间',werks,lgort,lgpla,matnr,maktx,lgtyp
,charg,sonum_ex,bestq,verme,quantity,username
from (
...
--select * from  v_material_history_item_log A
